...
async function createSupply(limit: number, braavTypeArg: string): Promise<any> {
    ...
    try {
        ...
        const createdBraav = result.objectChanges?.find(
            (change: any) => change.type === 'created' && change.objectType.includes('::braav_public::BRAAV')
        ) as { type: 'created'; objectId: string } | undefined;

        const createdLineage = result.objectChanges?.find(
            (change: any) => change.type === 'created' && change.objectType.includes('::braav_public::Lineage')
        ) as { type: 'created'; objectId: string } | undefined;

        const createdCounter = result.objectChanges?.find(
            (change: any) => change.type === 'created' && change.owner.AddressOwner === keypair.toSuiAddress() && change.objectType.includes('::counter::Counter')
        ) as { type: 'created'; objectId: string } | undefined;
        ...
        if (createdBraav?.objectId) {
            ...
            const content = braavObject.data?.content as any;
